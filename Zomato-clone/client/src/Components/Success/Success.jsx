...
const Success = () => {
	...
	useEffect(() => {
		async function fetchOrders() {
			try {
				const response = await axios.get('http://localhost:4001/api/past-orders');
	...
	const renderStars = (rating, productId) => {
		const stars = [];
		for (let i = 1; i <= 5; i++) {
			stars.push(<FontAwesomeIcon key={i} icon={faStar} color={i <= rating ? 'gold' : 'grey'} onClick={() => handleRating(productId, i)} style={{ cursor: 'pointer' }} />);
		}
		return stars;
	};
